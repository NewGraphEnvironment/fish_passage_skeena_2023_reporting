...
\@ref(tab:tab-culvert-bcfp-198022). A map of the watershed is provided in
map attachment
...
[@moe2011Fieldassessment] (Table \@ref(tab:tab-culvert-198022)). Although the culvert was slightly undersized for the size of the stream, it was equipped to handle major flows and was in good condition. Water temperature was `r fpr::fpr_my_habitat_info(loc = "ds", col_pull = 'temperature_c')`$^\circ$C,
...
During the 2023 survey, electrofishing was conducted at three sites below and one site above crossing `r as.character(my_site)`, with results summarised in Tables \@ref(tab:tab-fish-site-198022) - \@ref(tab:tab-fish-dens-198022) and Figure
\@ref(fig:plot-fish-box-198022). A total of `r tab_fish_summary %>% filter(site_id == paste0(my_site, '_us')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` fish were captured upstream and `r tab_fish_summary %>% filter(site_id == paste0(my_site, '_ds')) %>% group_by(site_id) %>% janitor::adorn_totals() %>% filter(site_id == 'Total') %>% pull(count_fish)` fish were captured downstream, all of which were dolly varden `r if(gitbook_on){knitr::asis_output("(Figures \\@ref(fig:photo-198022-03) - \\@ref(fig:photo-198022-04)).")}else(knitr::asis_output("(Figures \\@ref(fig:photo-198022-d02))."))` All fish captured with a fork length greater than 60mm were tagged with Passive Integrated Transponders (PIT tags) with data stored [here](`r paste0(params$repo_url, "blob/main/data/2023/habitat_confirmations.xls")`).
...
```{r photo-198022-04, fig.cap= my_caption2, out.width = photo_width, eval=gitbook_on}
knitr::include_graphics(my_photo2)
```
