...
PSCIS crossing `r as.character(my_site)` on a `r fpr::fpr_my_pscis_info()` is located `r round(fpr::fpr_my_bcfishpass(col_pull = downstream_route_measure)*0.001, 1)`km upstream of Coal Creek on `r fpr_my_pscis_info(col_pull = road_name)`.
...
`r fpr::fpr_my_wshd(col ='elev_min')`m near the crossing (Table \@ref(tab:tab-wshd-8525)).

<br>

The crossing was  assessed with a fish passage re-assessment in 2022 and ranked as a moderate priority for follow up
in @irvine_wintersheidt2023SkeenaWatershed due to relatively low mapped gradients, the crossing's proximity to Coal Creek, 
and decent flows. In 2022, minnow traps were set at crossing `r as.character(my_site)` to scope for coho presence with
only dolly varden were captured. During the 2023 survey, surveyors conducted a habitat confirmation assessment and 
electrofishing with results included in this memo. 
...
`r fpr_my_survey_dist(loc = 'ds')`. The stream had low flows with intermittent dewatering, but had good flow relative to
other streams in the area and given it was a very dry year. Flows in the stream were noted as relatively turbid
`r if(gitbook_on){knitr::asis_output("(Figure \\@ref(fig:photo-8525-01)).")}else(knitr::asis_output("(Figure \\@ref(fig:photo-8525-d01))."))` There were very few deep pools present bud abundant gravel present throughout the area surveyed.
...
Approximately 1.7km upstream of crossing `r as.character(my_site)`, PSCIS crossing `r as.character(my_site2)`-  which 
was previously documented as a wooden box culvert - was reassessed. The crossing had recently been replaced with 
a bridge. The stream was surveyed upstream from crossing `r as.character(my_site2)` for approximately 100m. The stream 
was noted as having good flow for a small system on a dry year. `r fpr_my_habitat_paragraph(loc = 'us2')` `r if(gitbook_on){knitr::asis_output("(Figure \\@ref(fig:photo-8529-01) - \\@ref(fig:photo-8529-02)).")}else(knitr::asis_output("(Figure \\@ref(fig:photo-8529-d01))."))` `r fpr_my_habitat_paragraph(loc = 'us2')` The habitat was rated as `r fpr::fpr_my_priority_info(loc = 'us2') %>% stringr::str_to_lower()` value for salmonid rearing and spawning. 
