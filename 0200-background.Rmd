...
through the [Open Government Portal](https://open.canada.ca/en) with results presented in Table \@ref(tab:tab-dfo-sad-tr-bulk). A brief memo on the data extraction process is available in the [memos directory](https://github.com/NewGraphEnvironment/restoration_wedzin_kwa_2024/tree/main/memos).


```{r tab-dfo-sad-tr-bulk, eval=TRUE}
...
dfo_sad_tr <- readr::read_csv('data/inputs_raw/All Areas NuSEDS.csv') %>% 
  # dplyr::filter(
  #   waterbody == "BULKLEY RIVER - UPPER" | 
  #     waterbody == "MORICE RIVER" ) %>% 
...
dfo_sad_tr %>%
  dplyr::filter(stringr::str_detect(waterbody,"BULKLEY")) |>
  # dplyr::select(-waterbody) |> 
  fpr::fpr_kable(caption_text = "Fisheries and Oceans Canada stock assessment data for the Bulkley River.  Data is filtered to only include records with total return to river data.",
                 scroll = gitbook_on)
```
...
#### Salmon Stock Assessment Data
Fisheries and Oceans Canada stock assessment data is presented in Table \@ref(tab:tab-dfo-sad-tr-morice). 

```{r tab-dfo-sad-tr-morice, eval=TRUE}
...
#### Salmon Stock Assessment Data
Fisheries and Oceans Canada stock assessment data is presented in Table \@ref(tab:tab-dfo-sad-tr-zymoetz).

```{r tab-dfo-sad-tr-zymoetz, eval=TRUE}

dfo_sad_tr %>%
  dplyr::filter(stringr::str_detect(waterbody,"ZYMOETZ")) |>
  # dplyr::select(-waterbody) |> 
  fpr::fpr_kable(caption_text = "Fisheries and Oceans Canada stock assessment data for the Zymoetz River.  Data is filtered to only include records with total return to river data.",
                 scroll = gitbook_on)
...
#### Salmon Stock Assessment Data
Fisheries and Oceans Canada stock assessment data is presented in Table \@ref(tab:tab-dfo-sad-tr-kispiox).

```{r tab-dfo-sad-tr-kispiox, eval=TRUE}

dfo_sad_tr %>%
  dplyr::filter(stringr::str_detect(waterbody,"KISPIOX")) |>
  dplyr::select(-waterbody) |>
  fpr::fpr_kable(caption_text = "Fisheries and Oceans Canada stock assessment data for the Zymoetz River.  Data is filtered to only include records with total return to river data.",
                 scroll = gitbook_on)
...
#### Salmon Stock Assessment Data
Fisheries and Oceans Canada stock assessment data is presented in Table \@ref(tab:tab-dfo-sad-tr-kitsumkalum).

```{r tab-dfo-sad-tr-kitsumkalum, eval=TRUE}

dfo_sad_tr %>%
  dplyr::filter(stringr::str_detect(waterbody,"KITSUMKALUM")) |>
  # dplyr::select(-waterbody) |> 
  fpr::fpr_kable(caption_text = "Fisheries and Oceans Canada stock assessment data for the Kitsumkalum River.  Data is filtered to only include records with total return to river data.",
                 scroll = gitbook_on)
